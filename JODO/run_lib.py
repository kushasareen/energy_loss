...
def vpsde_edge_train(config, workdir):
    ...
    run_name = f"{config.data.name}_lr{config.optim.lr}_bs{config.training.batch_size}_" \
           f"energy{int(config.training.use_energy_loss)}_align{int(config.model.noise_align)}_{config.training.coeff_mode}"
    wandb.init(project="jodo",
               name=run_name,
               dir=workdir,
               config=config)
    ...
    if config.training.snapshot_sampling:
        sampling_fn = get_sampling_fn(config, noise_scheduler, nodes_dist,
    ...
    for step in range(initial_step, num_train_steps + 1):
        ...
        if step % config.training.log_freq == 0:
            log_data = {"step": step, "training_loss": loss.item()}
        ...
        if step != 0 and step % config.training.snapshot_freq == 0 or step == num_train_steps:
            ...
            if config.training.snapshot_sampling:
                ...
                # 3D evaluation
                stability_3d, rdkit_3d, sample_rdmols = EDM_metric(processed_mols)
                fcd_3d = fcd_metric(sample_rdmols)
                log_data_3d = {
                    "step": step,
                    "n_mol_3D": len(sample_rdmols),
                    "3D_atom_stability": stability_3d['atom_stable'],
                    "3D_mol_stability": stability_3d['mol_stable'],
                    "3D_validity": rdkit_3d['Validity'],
                    "3D_complete": rdkit_3d['Complete'],
                    "3D_unique_valid": rdkit_3d['Unique'],
                    "3D_FCD": fcd_3d['FCD']
                }
                logging.info("3D metrics: " + str(log_data_3d))
                wandb.log(log_data_3d)

                # 2D evaluation
                stability_2d, rdkit_2d, complete_rdmols = EDM_metric_2D(processed_mols)
                fcd_2d = fcd_metric(complete_rdmols)
                log_data_2d = {
                    "n_mol_2D": len(complete_rdmols),
                    "2D_atom_stability": stability_2d['atom_stable'],
                    "2D_mol_stability": stability_2d['mol_stable'],
                    "2D_validity": rdkit_2d['Validity'],
                    "2D_complete": rdkit_2d['Complete'],
                    "2D_unique_valid": rdkit_2d['Unique'],
                    "2D_FCD": fcd_2d['FCD']
                }
                logging.info("2D metrics: " + str(log_data_2d))
                wandb.log(log_data_2d)
