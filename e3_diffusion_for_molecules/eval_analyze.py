...
def analyze_and_save(args, eval_args, device, generative_model,
                     ...
                     batch_size=10, save_to_xyz=False, save_to_sdf=True):
    ...
    if save_to_sdf:
        print("Saving molecules to sdf file.")
        num_molecules = 0
        from rdkit.Chem.rdmolfiles import SDWriter
        writer = SDWriter(join(eval_args.model_path, 'molecules.sdf'))
        for i in range(molecules['x'].shape[0]):
            try:
                mol = rdkit_functions.build_molecule(
                    positions=molecules['x'][i],
                    atom_types=torch.argmax(molecules['one_hot'][i], dim=1),
            ...
            if mol is not None:
                ...
                num_molecules += 1
        writer.close()
        print(f"Saved {num_molecules} molecules to {join(eval_args.model_path, 'molecules.sdf')}")

        # additionally save molecules to a pickle file
        with open(join(eval_args.model_path, 'molecules.pickle'), 'wb') as f:
...
def main():
    ...
    parser.add_argument('--save_to_sdf', type=eval, default=False,
                        help='Should save samples to sdf files.')
    ...
    stability_dict, rdkit_metrics = analyze_and_save(
        ...
        batch_size=eval_args.batch_size_gen, save_to_xyz=eval_args.save_to_xyz, save_to_sdf=eval_args.save_to_sdf)
