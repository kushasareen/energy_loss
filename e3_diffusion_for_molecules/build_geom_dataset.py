...
from rigidity.generate_sym_dataset_geom import generate_symmetric_dataset, generate_k_regular_graphs
...
class GeomDrugsDataset(Dataset):
    def __init__(self, data_list, split, dataset_info, transform=None, add_sym_edges=False, idx=-1, data_path = ''):
        """
        Args:
        ...
        self.add_sym_edges = add_sym_edges
        self.dataset_info = dataset_info

        if self.add_sym_edges:
            # just going to do k regular graphs for now
            import pickle
            filename = f'{data_path}/sym_edges/k_regular.pickle'
            if os.path.isfile(filename):
                with open(filename, 'rb') as handle:
                    self.k_regular = pickle.load(handle)
                print(f'Loaded k_regular graphs from {filename}')
            else:
                print(f'Generating k_regular graphs for {split} dataset')
                self.k_regular = generate_k_regular_graphs(100, range(2, 200), 12)
                # save the k-regular graphs
                with open(filename, 'wb') as handle:
                    pickle.dump(self.k_regular, handle)
                print(f'Saved k_regular graphs to {filename}')
    ...
    def __getitem__(self, idx):
        ...
        if self.add_sym_edges:
            ...
            num_atoms_idx = num_atoms - 2
            random_idx = np.random.randint(0, 100)
            sample['sym_edges'] = self.k_regular[num_atoms_idx][random_idx]
...
def collate_fn(in_batch):
    stack_keys = [key for key in in_batch[0].keys() if key != 'sym_edges']
    batch = {prop: qm9_collate.batch_stack([mol[prop] for mol in in_batch])
             for prop in stack_keys}
    ...
    if 'sym_edges' in in_batch[0]:
        # want to pad all the adjacency matrices to the same size (n_nodes x n_nodes)
        sym_edges_batch = [
            F.pad(mol['sym_edges'], (0, n_nodes - mol['sym_edges'].size(1),
                   0, n_nodes - mol['sym_edges'].size(0)),
                   value=0) for mol in in_batch]
        batch['sym_edges'] = torch.stack(sym_edges_batch)
