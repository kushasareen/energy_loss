...
from utils import add_bonds, add_k_regular
from configs.datasets_config import get_dataset_info
from rigidity.generate_sym_dataset import generate_symmetric_dataset


def initialize_datasets(args, datadir, dataset, subset=None, splits=None,
                        force_download=False, subtract_thermo=False,
                        remove_h=False, add_edges=False, k_regular=False, add_sym_edges=False):
    """
    Initialize datasets.
    ...
    if add_edges:
        for split, dataset in datasets.items():
            if os.path.isfile(f'edges_{split}.pt'):
                print(f'Loading edges from edges_{split}.pt')
                dataset.data['edges'] = torch.load(f'edges_{split}.pt')
            else:
                dataset.data['edges'] = add_bonds(dataset.data['positions'], dataset.data['one_hot'], dataset_info = get_dataset_info(args.dataset, remove_h), all_num_atoms = dataset.data['num_atoms'])
                torch.save(dataset.data['edges'], f'edges_{split}.pt')

    if add_sym_edges:
        for split, dataset in datasets.items():
            if os.path.isfile(f'sym_edges_{split}.pt'):
                print(f'Loading sym_edges from sym_edges_{split}.pt')
                dataset.data['sym_edges'] = torch.load(f'sym_edges_{split}.pt')
            else:
                dataset.data['sym_edges'] = generate_symmetric_dataset(dataset.data, k = 6, size = 29) # QM9
                # dataset.data['sym_edges'] = generate_symmetric_dataset(dataset.data, k = 6, size = 60) 
                torch.save(dataset.data['sym_edges'], f'sym_edges_{split}.pt')
