...
parser.add_argument('--energy_loss_weight', type=float, default=0.03)
parser.add_argument('--energy_coeff_mode', type=str, default='exp_dist')
parser.add_argument('--seed', type=int, default=42)
parser.add_argument('--pick_best_hyperparams', type=bool, default=False)
parser.add_argument('--dataset_frac', type=float, default=1.0)
parser.add_argument('--add_edges', type=bool, default=False)
parser.add_argument('--add_sym_edges', type=bool, default=False)
parser.add_argument('--k_regular', type=bool, default=False)
parser.add_argument('--energy_edge_mode', type=str, default='complete')
parser.add_argument('--run_eval', type=bool, default=True)
parser.add_argument('--hybrid_mode', type=str, default='mae')
parser.add_argument('--leq_atoms', type=int, default=None, help='When set to an integer value, QM9 will only contain molecules with less or equal amount of atoms')
parser.add_argument('--loss_mode', type=int, default="mse")
parser.add_argument('--save_path', type=str, default='./outputs')
parser.add_argument('--save_every_iter', type=int, default=None)
parser.add_argument('--idx', type=int, default=-1, help='For symmetric edge creation for GEOM.')
...
if args.pick_best_hyperparams:
    args = utils.pick_best_hyperparams(args)
    args.exp_name = 'geom_' + utils.get_unique_name(args)
else:
    args.exp_name = 'geom_' + utils.get_unique_name(args) + '_lr' + str(args.lr)

if args.add_sym_edges:
    args.exp_name += '_sym_edges2'
...
for key, data_list in zip(['train', 'val', 'test'], split_data):
    print(f'{key} size: {len(data_list)}')
    dataset = build_geom_dataset.GeomDrugsDataset(data_list, transform=transform, split = key, dataset_info=dataset_info, add_sym_edges=args.add_sym_edges, idx=args.idx)
    shuffle = (key == 'train') and not args.sequential
    # if args.add_sym_edges:
    #     for idx in range(len(dataset)):
    #         n = len(dataset[idx]['atom_mask'])
    #         dataset[idx]['num_atoms'] = n
    #         sym_edge = 
...
kwargs = {'entity': args.wandb_usr, 'name': args.exp_name, 'project': 'e3_diffusion_geom', 'config': args, 'id': args.exp_name, 'resume': "allow",
        #   'settings': wandb.Settings(_disable_stats=True), 
          'reinit': True, 'mode': mode}

print("Initializing wandb...")
print("Wandb name: ", args.exp_name)
wandb.init(**kwargs)
wandb.save('*.txt')

data_dummy = next(iter(dataloaders['train']))
# with open("geom_data_dummy.pickle", "wb") as f:
    ...
    for epoch in range(args.start_epoch, args.n_epochs):
        ...
        if epoch % args.test_epochs == 0:
            ...
            if args.save_model:
                ...
                utils.save_model(optim, join(args.save_path, args.exp_name, 'optim.npy'))
                utils.save_model(model, join(args.save_path, args.exp_name, 'generative_model.npy'))
                if args.ema_decay > 0:
                    utils.save_model(model_ema, join(args.save_path, args.exp_name, 'generative_model_ema.npy'))
                with open(join(args.save_path, args.exp_name, 'args.pickle'), 'wb') as f:
                    pickle.dump(args, f)

            if args.save_model:
                utils.save_model(optim, join(args.save_path, args.exp_name, f'optim_{epoch}.npy'))
                utils.save_model(model, join(args.save_path, args.exp_name, f'generative_model_{epoch}.npy'))

                if args.ema_decay > 0:
                    utils.save_model(model_ema, join(args.save_path, args.exp_name, f'generative_model_ema_{epoch}.npy'))
                with open(join(args.save_path, args.exp_name, f'args_{epoch}.pickle'), 'wb') as f:
                    pickle.dump(args, f)
            ...
            import sys
            sys.exit() # we end after one epoch to let the resume script take over
